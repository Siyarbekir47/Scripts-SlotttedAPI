...
cheat.register_module({
    ...
    spell_q = function()
        ...
        print("9")
        if((slot_spell_w:is_ready() == false and slot_spell_e:is_ready() == false and features.orbwalker:get_mode() == 1 and qHit.hitchance > 1.0 and g_time - g_last_q_time > 0.35) or (qHit.hitchance > 1.0 and features.orbwalker:get_mode() == 4 and target:dist_to_local() < spell_q_range and g_time - g_last_q_time > 0.35) and combo_q_box:get_value()) then
            print("1")
            g_last_q_time = g_time
    ...
    spell_w = function()
        ...
        --if((features.orbwalker:get_mode() == 1  and target:dist_to_local() < spell_w_range) or (features.orbwalker:get_mode() == 1 and target:dist_to_local() < spell_w_range and target:dist_to_local() > 290))
       -- if(features.orbwalker:get_mode() == 1 and g_local.health < low_health and target:dist_to_local() < spell_w_range ) then
        --    g_input:cast_spell(e_spell_slot.w, target.network_id)
       -- end
